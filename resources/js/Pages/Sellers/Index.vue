        ...
        <div class="container pb-16">
            <div class="flex flex-col justify-between flex-1 p-4 my-8 bg-white rounded-lg shadow-md md:flex-row">
                <div class="flex items-center justify-center mb-4 md:mb-0">
                    <h2 class="text-xl md:text-2xl font-medium font-roboto text-gray-800 uppercase">Stores</h2>
                </div>
            ...
            <div class="grid lg:grid-cols-8 md:grid-cols-6 grid-cols-3 lg:gap-6 md:gap-4 gap-2">
                <!-- single brand -->
                <div v-for="seller in sellers.data" class="group flex flex-col rounded bg-white shadow overflow-hidden">
                    <!-- brand image -->
                    <div class="relative p-2">
                        <img :src="seller.store_logo" class="w-full">
                    ...
                    <!-- brand image end -->
                    <inertia-link :href="route('sellers.show', seller.slug)">
                        <h4 class="font-medium text-sm text-center font-roboto line-clamp-2 mb-2 text-gray-800 hover:text-primary transition">{{ seller.store_name }}</h4>
                    </inertia-link>
                </div>
                <!-- single brand end -->
            </div>

            <template v-if="sellers.meta.total > sellers.meta.per_page">
