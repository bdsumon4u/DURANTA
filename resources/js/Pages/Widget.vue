        ...
        <div class="container pb-16">
            <div class="flex flex-col justify-between flex-1 p-4 my-8 bg-white rounded-lg shadow-md md:flex-row">
                <div class="flex items-center justify-center mb-4 md:mb-0">
                    <h2 class="text-xl md:text-2xl font-medium font-roboto text-gray-800 uppercase">
                        <inertia-link :href="route('widget', {slug: widget.data.slug})">{{ widget.data.title }}</inertia-link>
                    </h2>
                </div>
...
export default {
    ...
    methods: {
        search() {
            this.form.get(route('widget', {slug: this.widget.data.slug}));
        }
    },
    data() {
        return {
