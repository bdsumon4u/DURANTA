...
class Client
{
    ...
    public static function initSession(Customer $customer, $amount, $config=[]){
        ...
        $data[SessionRequest::CUSTOMER_ADDRESS_1] = $customer->getAddress1();
        $data[SessionRequest::CUSTOMER_CITY] = $customer->getCity();
        $data[SessionRequest::CUSTOMER_STATE] = $customer->getState();
        $data[SessionRequest::CUSTOMER_POSTCODE] = $customer->getPostCode();
        $data[SessionRequest::CUSTOMER_COUNTRY] = $customer->getCountry();
        $data[SessionRequest::CUSTOMER_FAX] = $customer->getFax();
